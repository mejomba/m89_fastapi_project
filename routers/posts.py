...
def home_page(request: Request, db: Session = Depends(get_db)):
    last_post = db.query(models.posts.Post).order_by(desc(models.posts.Post.last_update)).limit(100)
...
def get_post(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    # post_and_comments = dict()
    # post_and_comments.update({"post": post, "comments": comments})
